...
`qcom/sm8750/<card model>-tplg.bin`. The two SM8750 boards ship one each:

| card model     | board             | blob                                                                    |
|----------------|-------------------|-------------------------------------------------------------------------|
| `SM8750-AYN`   | AYN Odin3         | `filesystem/usr/lib/kernel-overlays/base/lib/firmware/qcom/sm8750/SM8750-AYN-tplg.bin`   |
| `SM8750-KONKR` | KONKR Pocket FIT Elite | `filesystem/usr/lib/kernel-overlays/base/lib/firmware/qcom/sm8750/SM8750-KONKR-tplg.bin` |

The blobs in `filesystem/` override the ones the `extra-firmware` package
installs (the device overlay is copied last). The KONKR blob is additionally
built into the kernel via `CONFIG_EXTRA_FIRMWARE`, see the SM8750 block in
